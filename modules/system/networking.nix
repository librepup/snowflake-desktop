      ...
      dispatcherScripts = [
        {
          source = pkgs.writeShellScript "connectivityUpdateHook" ''
            if [ "$2" = "up" ] || [ "$2" = "down" ] || [ "$2" = "connectivity-change" ]; then
              case "$1" in
                enp0s31f6|enp*)
                  ${pkgs.sudo}/bin/sudo -u "puppy" DISPLAY=":0" DBUS_SESSION_BUS_ADDRESS="unix:path=/run/user/$(${pkgs.coreutils}/bin/id -u puppy)/bus" ${pkgs.libnotify}/bin/notify-send "$1" "$1: $2" -i /run/current-system/sw/share/icons/breeze-dark/status/24/network-wired.svg
                  ;;
                wlp4s0u2|wlp*)
